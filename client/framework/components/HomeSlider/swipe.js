...
(function (root, factory) {
    ...
    return function Swipe (container, options) {
        ...
        function slide(to, slideSpeed, direction) {
            ...
            //更改title
            fadeTitle(titleNode, options.sliderData[to]['recommend_content']);
        ...
        var events = {
            ...
            move: function(event) {
                ...
                if (!delta.x && !delta.y) {
                    try {
                        options.sudaFunc && options.sudaFunc("crowdfund_home_h5", "slide:" + (options.sliderData[index]['banner_id'] || 0)+":"+encodeURIComponent(options.sliderData[index]['recommend_content'] || ""));
                    } catch (e) {
                    }
                    var _url = slides[index].getAttribute("data-href");
                    _url && (doJumpPage(_url));
                }
